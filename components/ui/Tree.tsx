  ...
  return (
    <AriaTree
      ...
      className={composeTailwindRenderProps(
        props.className,
        "w-48 max-w-full overflow-auto relative border border-outline-variant rounded-lg bg-surface",
      )}
    >
      {children}
    </AriaTree>
      ...
      <AriaTreeItemContent {...props}>
        {({ selectionMode, selectionBehavior, hasChildItems, isExpanded, isDisabled }) => (
          <div className={`flex items-center`}>
            ...
            <div className="shrink-0 w-[calc(calc(var(--tree-item-level)-1)*--spacing(3))]" />
            {hasChildItems ? (
