...
export function ListBox<T extends object>(
  ...
  return (
    <AriaListBox {...props} className={composeTailwindRenderProps(props.className, 'outline-0 p-1 w-[200px] bg-surface border border-outline-variant rounded-lg font-sans')}>
      {children}
    </AriaListBox>
  );
}
...
export const itemStyles = tv({
  ...
  variants: {
    isSelected: {
      false: 'text-on-surface hover:bg-surface-container-high pressed:bg-surface-container-highest -outline-offset-2',
...
export function DropdownSection<T extends object>(props: DropdownSectionProps<T>) {
  return (
    <ListBoxSection className="first:-mt-[5px] after:content-[''] after:block after:h-[5px] last:after:hidden">
      <Header className="text-sm font-semibold text-on-surface-variant px-4 py-1 truncate sticky -top-[5px] -mt-px -mx-1 z-10 bg-surface-container/60 backdrop-blur-md border-y border-outline-variant [&+*]:mt-1">{props.title}</Header>
