...
export function GridList<T extends object>(
  ...
  return (
    <AriaGridList {...props} className={composeTailwindRenderProps(props.className, 'overflow-auto w-[200px] relative bg-surface border border-outline-variant rounded-lg font-sans empty:flex empty:items-center empty:justify-center empty:italic empty:text-sm empty:text-on-surface-variant')}>
      {children}
    </AriaGridList>
  );
}
...
export function GridListHeader({children, ...props}: HTMLAttributes<HTMLElement>) {
  return (
    <AriaGridListHeader {...props} className={twMerge("text-sm font-semibold text-on-surface-variant px-4 py-1 -mt-px z-10 bg-surface-container/60 backdrop-blur-md border-y border-outline-variant", props.className)}>{children}</AriaGridListHeader>
  )
}
