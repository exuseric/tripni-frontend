...
export function Table(props: TableProps) {
  return (
    <ResizableTableContainer onScroll={props.onScroll} className={twMerge('w-full max-h-[320px] overflow-auto scroll-pt-[2.281rem] relative bg-surface box-border border border-outline-variant rounded-lg font-sans', props.className)}>
...
export function Column(props: ColumnProps) {
  return (
    <AriaColumn {...props} className={composeTailwindRenderProps(props.className, 'box-border h-1 [&:hover]:z-20 focus-within:z-20 text-start text-sm font-semibold text-on-surface cursor-default')}>
        ...
        <div className="flex items-center">
          ...
          >
            ...
            {allowsSorting && (
              ...
              >
                {sortDirection && <ArrowUp aria-hidden className="w-4 h-4 text-on-surface-variant forced-colors:text-[ButtonText]" />}
              </span>
            )}
          </Group>
...
export function TableHeader<T extends object>(props: TableHeaderProps<T>) {
  ...
  return (
    <AriaTableHeader {...props} className={composeTailwindRenderProps(props.className, 'sticky top-0 z-10 bg-surface-container/60 backdrop-blur-md rounded-t-lg border-b border-b-outline-variant')}>
