...
export function Label(props: LabelProps) {
  return (
    <RACLabel
      {...props}
      className={twMerge(
        "font-sans text-sm text-on-surface font-medium cursor-default w-fit",
        props.className,
      )}
    />
  );
...
export function Description(props: TextProps) {
  return (
    <Text
      {...props}
      slot="description"
      className={twMerge("text-sm text-on-surface-variant", props.className)}
    />
  );
}
...
export function Input(props: InputProps) {
  return (
    <RACInput
      ...
      className={composeTailwindRenderProps(
        props.className,
        "px-3 py-0 min-h-9 flex-1 min-w-0 border-0 outline bg-transparent font-sans text-sm text-on-surface placeholder:text-on-surface-variant/50 disabled:text-on-surface/20 disabled:placeholder:text-on-surface-variant/10 [-webkit-tap-highlight-color:transparent]",
      )}
    />
  );
}
