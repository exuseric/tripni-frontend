    ...
    <AriaColorSwatchPickerItem {...props} className={itemStyles}>
      {({ isSelected }) => (
        <>
          <ColorSwatch />
          {isSelected && (
            <div className="absolute top-0 left-0 w-full h-full box-border border-2 border-on-surface outline-2 outline-surface -outline-offset-4 rounded-md forced-color-adjust-none" />
          )}
        </>
      )}
